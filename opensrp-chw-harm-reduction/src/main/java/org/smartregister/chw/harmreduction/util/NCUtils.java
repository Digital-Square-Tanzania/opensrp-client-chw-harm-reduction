...
public class NCUtils {
    ...
    public static org.smartregister.Context context() {
        return HarmReductionLibrary.getInstance().context();
    }
    ...
    public static SimpleDateFormat getSourceDateFormat() {
        return new SimpleDateFormat(HarmReductionLibrary.getInstance().getSourceDateFormat(), Locale.getDefault());
    }

    public static SimpleDateFormat getSaveDateFormat() {
        return new SimpleDateFormat(HarmReductionLibrary.getInstance().getSaveDateFormat(), Locale.getDefault());
    }
    ...
    public static void processHomeVisit(EventClient baseEvent, SQLiteDatabase database, String parentEventType) {
        try {
            ...
            if (visit == null) {
                ...
                if (database != null) {
                    HarmReductionLibrary.getInstance().visitRepository().addVisit(visit, database);
                } else {
                    HarmReductionLibrary.getInstance().visitRepository().addVisit(visit);
                }
                    ...
                    for (Map.Entry<String, List<VisitDetail>> entry : visit.getVisitDetails().entrySet()) {
                        if (entry.getValue() != null) {
                            for (VisitDetail detail : entry.getValue()) {
                                if (database != null) {
                                    HarmReductionLibrary.getInstance().visitDetailsRepository().addVisitDetails(detail, database);
                                } else {
                                    HarmReductionLibrary.getInstance().visitDetailsRepository().addVisitDetails(detail);
                                }
                            }
                        }
                    }

...
import org.smartregister.chw.harmreduction.repository.VisitDetailsRepository;
...
public class VisitUtils {
    ...
    public static List<Visit> getVisits(String memberID, String... eventTypes) {
        ...
        List<Visit> visits = (eventTypes != null && eventTypes.length > 0) ? getVisitsOnly(memberID, eventTypes[0]) : getVisitsOnly(memberID, Constants.EVENT_TYPE.HARM_REDUCTION_RISK_ASSESSMENT);
    ...
    public static List<Visit> getVisitsOnly(String memberID, String visitName) {
        return new ArrayList<>(HarmReductionLibrary.getInstance().visitRepository().getVisits(memberID, visitName));
    }

    public static List<VisitDetail> getVisitDetailsOnly(String visitID) {
        return HarmReductionLibrary.getInstance().visitDetailsRepository().getVisits(visitID);
    }
    ...
    public static void processVisits(String baseEntityID) throws Exception {
        processVisits(HarmReductionLibrary.getInstance().visitRepository(), HarmReductionLibrary.getInstance().visitDetailsRepository(), baseEntityID);
    }
    ...
    public static void processVisits(List<Visit> visits, VisitRepository visitRepository, VisitDetailsRepository visitDetailsRepository) throws Exception {
        ...
        for (Visit v : visits) {
            if (!v.getProcessed()) {
                ...
                baseEvent.addDetails(Constants.HARM_REDUCTION_VISIT_GROUP, visitGroupId);

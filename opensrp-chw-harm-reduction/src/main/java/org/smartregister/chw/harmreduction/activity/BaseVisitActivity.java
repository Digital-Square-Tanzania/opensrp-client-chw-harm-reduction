...
import org.smartregister.chw.harmreduction.adapter.BaseHarmReductionVisitAdapter;
...
public class BaseVisitActivity extends SecuredActivity implements BaseHarmReductionVisitContract.View, View.OnClickListener {

    private static final String TAG = BaseVisitActivity.class.getCanonicalName();
    protected Map<String, BaseHarmReductionVisitAction> actionList = new LinkedHashMap<>();
    protected BaseHarmReductionVisitContract.Presenter presenter;
    ...
    public static void startMe(Activity activity, String baseEntityID, Boolean isEditMode) {
        Intent intent = new Intent(activity, BaseVisitActivity.class);
    ...
    protected MemberObject getMemberObject(String baseEntityId) {
        return HarmReductionDao.getMember(baseEntityId);
    }
    ...
    public void setUpView() {
        ...
        mAdapter = new BaseHarmReductionVisitAdapter(this, this, (LinkedHashMap) actionList);
    ...
    protected void registerPresenter() {
        presenter = new BaseHarmReductionVisitPresenter(memberObject, this, new BaseHarmReductionVisitInteractor());
    }


    @Override
    public void initializeActions(LinkedHashMap<String, BaseHarmReductionVisitAction> map) {
        actionList.clear();
    ...
    @Override
    public BaseHarmReductionVisitContract.Presenter presenter() {
        return presenter;
    }
    ...
    public void startForm(BaseHarmReductionVisitAction tbleprosyHomeVisitAction) {
        ...
        if (StringUtils.isNotBlank(tbleprosyHomeVisitAction.getJsonPayload())) {
            try {
                ...
                Timber.e(e);
                String locationId = HarmReductionLibrary.getInstance().context().allSharedPreferences().getPreference(AllConstants.CURRENT_LOCATION_ID);
    ...
    @Override
    public void startFragment(BaseHarmReductionVisitAction tbleprosyHomeVisitAction) {
        current_action = tbleprosyHomeVisitAction.getTitle();
    ...
    public void redrawVisitUI() {
        ...
        for (Map.Entry<String, BaseHarmReductionVisitAction> entry : actionList.entrySet()) {
            ...
            if (!action.isEnabled()
                    || (!action.isOptional() && action.getActionStatus() == BaseHarmReductionVisitAction.Status.PENDING)) {
    ...
    @Override
    public Map<String, BaseHarmReductionVisitAction> getTbLeprosyVisitActions() {
        return actionList;
    }
    ...
    @Override
    public BaseHarmReductionVisitContract.Presenter getPresenter() {
        return presenter;
    }
    ...
    public void onDialogOptionUpdated(String jsonString) {
        BaseHarmReductionVisitAction tbleprosyVisitAction = actionList.get(current_action);
        ...
        if (requestCode == Constants.REQUEST_CODE_GET_JSON) {
            if (resultCode == Activity.RESULT_OK) {
                try {
                    ...
                    BaseHarmReductionVisitAction baseTbLeprosyVisitAction = actionList.get(current_action);
    ...
    public void onBackPressed() {
        displayExitDialog(BaseVisitActivity.this::finish);
    }

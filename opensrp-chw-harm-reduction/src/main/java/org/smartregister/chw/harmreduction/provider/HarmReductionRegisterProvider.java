package org.smartregister.chw.harmreduction.provider;
...
public class HarmReductionRegisterProvider implements RecyclerViewProvider<HarmReductionRegisterProvider.RegisterViewHolder> {
    ...
    private String updateMemberGender(CommonPersonObjectClient commonPersonObjectClient) {
        if ("0".equals(Utils.getValue(commonPersonObjectClient.getColumnmaps(), DBConstants.KEY.IS_ANC_CLOSED, false))) {
            ...
            return HarmReductionUtil.getGenderTranslated(context, gender);
        }
    }
    ...
    private void populatePatientColumn(CommonPersonObjectClient pc, final RegisterViewHolder viewHolder) {
        try {
            ...
            viewHolder.patientColumn.setTag(R.id.VIEW_ID, BaseHarmReductionRegisterFragment.CLICK_VIEW_NORMAL);

            viewHolder.dueButton.setOnClickListener(onClickListener);
            viewHolder.dueButton.setTag(pc);
            viewHolder.dueButton.setTag(R.id.VIEW_ID, BaseHarmReductionRegisterFragment.FOLLOW_UP_VISIT);
